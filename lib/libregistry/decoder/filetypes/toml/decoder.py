...
class TomlDecoder(FileTypeDecoder):
    """Decoder for TOML configuration files"""

    def decode(self, file_content: str, structure: Dict[str, Any]) -> Dict[str, Any]:
        """Decode TOML file content"""
        if tomllib is None:
            raise DecodingError("TOML support requires 'tomli' package (Python < 3.11)")
        try:
            data = tomllib.loads(file_content)
            return data
        except Exception as e:
            raise DecodingError(f"Failed to decode TOML: {e}")
    ...
    def validate(self, data: Dict[str, Any], structure: Dict[str, Any]) -> List[str]:
        """Validate TOML data against structure definition"""
        errors = []
